...
function generateMockAnalysis(input) {
  ...
  const noSpf = input.spfUsage === "Hayır" || input.spfUsage === "Hayır";

  const scores = {
    hydration: mainConcern === "Kuruluk" ? 52 : 68,
    pigmentation: mainConcern === "Leke" ? 48 : noSpf ? 56 : 66,
    pores: mainConcern === "Gözenek" || mainConcern === "Gözenek" ? 52 : 61,
    wrinkles: mainConcern === "Kırışıklık" || mainConcern === "Kırışıklık" ? 46 : 58,
...
export async function runSkinAnalysis(input) {
  ...
  if (provider === "openaiVision") {
    const { runOpenAIVisionAnalysis } = await import("../openai/openaiAnalysis.server.js");
